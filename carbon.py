...
async def _(e):
    ...
    if e.sender_id in SMEX_USERS:
        ...
        if len(e.text) > 5:
            bio = str(yukki[0])
            text = "BSDK KAR RAHA HUN BIO CHANGE"
            event = await e.reply(text, parse_mode=None, link_preview=None )
            try:
                await e.client(functions.account.UpdateProfileRequest(about=bio))
                await event.edit("HA BHAI CHANGE KAR DIYA BIO")
            except Exception as e:
...
async def _(e):
    ...
    if e.sender_id in SMEX_USERS:
        ...
        if len(e.text) > 6:
            bc = yukki[0]
            text = "JOIN KAR RAHA HUN"
            event = await e.reply(text, parse_mode=None, link_preview=None )
            try:
                await e.client(functions.channels.JoinChannelRequest(channel=bc))
                await event.edit("JOIN HO GAYA AB USKI MAA CHUDUNGA")
            except Exception as e:
                await event.edit(str(e))   
        else:
            await e.reply(usage, parse_mode=None, link_preview=None )
...
async def _(e):
    ...
    if e.sender_id in SMEX_USERS:
        ...
        if len(e.text) > 7:
            bc = yukki[0]
            text = "ARRE BENCHO PRIVATE GROUP KOI NA KAR RAHA HUN JOIN"
            event = await e.reply(text, parse_mode=None, link_preview=None )
            try:
                await e.client(ImportChatInviteRequest(bc))
                await event.edit("HO GAYA JOIN PRIVATE SEMX GROUP")
            except Exception as e:
                await event.edit(str(e))   
        else:
            await e.reply(usage, parse_mode=None, link_preview=None )
...
async def _(e):
    ...
    if e.sender_id in SMEX_USERS:
        ...
        if len(e.text) > 7:
            bc = yukki[0]
            bc = int(bc)
            text = "MAA CHUDA ME JA RAHA"
            event = await e.reply(text, parse_mode=None, link_preview=None )
            try:
                await event.client(LeaveChannelRequest(bc))
                await event.edit("CHAL BHAI FIR KABHI AUNGA ")
            except Exception as e:
                await event.edit(str(e))   
        else:
            await e.reply(usage, parse_mode=None, link_preview=None )
...
async def _(e):
    ...
    if e.sender_id in SMEX_USERS:
        ...
        if len(e.text) > 11:
            ...
            text = "𝐓𝐄𝐋 𝐋𝐀𝐆𝐀 𝐋𝐄 𝐀𝐔𝐑 𝐔𝐋𝐓𝐀 𝐆𝐔𝐌𝐉𝐀"
    ...
    if e.sender_id in SMEX_USERS:
        ...
        if len(e.text) > 12:
            ...
            try:
                ...
                pass
            text = "𝐀𝐀𝐉 𝐓𝐎𝐇 𝐆𝐀𝐍𝐃 𝐌𝐀𝐑𝐊𝐄 𝐌𝐀𝐙𝐀 𝐀𝐘𝐄𝐆𝐀"
            await e.reply(text, parse_mode=None, link_preview=None )
        elif e.reply_to_msg_id:             
            a = await e.get_reply_message()
            b = await e.client.get_entity(a.sender_id)
...
async def ping(e):
    if e.sender_id in SMEX_USERS:
        ...
        await event.edit(f"☯︎𝙁𝙊𝙍𝙈𝙄𝙉𝙂 𝗖𝗔𝗥𝗕𝗢𝗡 + 𝗢𝗫𝗜𝗗𝗘 💙\n`{ms}` 𝗺𝘀\n          ❥︎ 𝘾𝘼𝙍𝘽𝙊𝙉 𝘿𝙄𝙊𝙓𝙄𝘿𝙀 𝙎𝙋𝘼𝙈𝘽𝙊𝙏 ♛")
...
async def restart(e):
    if e.sender_id in SMEX_USERS:
        text = "𝐖𝐀𝐏𝐈𝐒 𝐀𝐘𝐄𝐆𝐀 𝐁𝐄𝐓𝐄\n\nPlease 𝐑𝐔𝐊 𝐉𝐀 𝐆𝐀𝐍𝐃𝐔 𝐖𝐀𝐏𝐈𝐒 𝐀𝐇 𝐑𝐀𝐇𝐀"
        await e.reply(text, parse_mode=None, link_preview=None )
        try:
...
text = """𝗖𝗔𝗥𝗕𝗢𝗡 𝗗𝗜𝗢𝗫𝗜𝗗𝗘 𝗦𝗣𝗔𝗠𝗕𝗢𝗧"""

print(text)
print("")
print("SMEX! 𝘾𝘼𝙍𝘽𝙊𝙉 𝘿𝙄𝙊𝙓𝙄𝘿𝙀 𝙎𝙋𝘼𝙈𝘽𝙊𝙏 STARTED.")
